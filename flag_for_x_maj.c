...
int		ft_flag_tiret_x_maj(int i, const char *s,unsigned int nombre_charact_int)
{
	...
	while (s[y] != 'X')
	{
		if (s[y] == '.')
		{
			double_flag = ft_flag_point_x_maj(y + 1, s,nombre_charact_int);
...
int		ft_ecriture_point_and_zero_x_maj(int i, int y, char *dest, int len_int)
{	
	if (y > len_int)
	{
		y = y - len_int;
...
int		ft_flag_zero_x_maj(int i, const char *s,int nombre_charact_int)
{
	...
	while (s[y] != 'X')
	{
		if (s[y] == '.')
		{
			...
			{
				ft_ecriture_point_and_zero_x_maj(y, tampon, dest, len_int);
				while (s[i] != 'X')
				{
					i++;
				}
