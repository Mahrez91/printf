...
int		ft_print_for_x_maj(int etoile,int etoile2, const char *s, int i, unsigned int nombre_charact_int)
{
	int c;
	...
	if (s[i] == '0')
	{
		...
		if (s[c] == '*' && s[c + 1] == '.' && s[c + 2] == '*')
		{
			...
			{
				i = ft_flag_etoile_tiret_point_x_maj(i, etoile , etoile - 1, nombre_charact_int) + 4;
				return (i);
			}
			if (etoile2 < 0)
				etoile2 = etoile;
			if ( etoile < 0)
			{
				if (etoile2 == 0 && nombre_charact_int == 0)
					etoile--;
				i = ft_flag_etoile_tiret_point_x_maj(i, etoile * (-1), etoile2, nombre_charact_int) + 4;
			}
			else
			{
				...
				if (etoile2 == 0 && nombre_charact_int == 0)
					etoile++;
				i = ft_flag_etoile_zero_point_x_maj(c, etoile, etoile2, nombre_charact_int) + 3;
			}
		}
	}
	if (s[i] == '*' && s[i + 2] != '*')
	...
	if (s[i] == '-' && s[i + 1] == '*' && s[i+2] == '.' && s[i+3] == '*')
	{
		...
		if (etoile < 0)
			etoile = etoile * (-1);
		i = ft_flag_etoile_tiret_point_x_maj(i, etoile, etoile2, nombre_charact_int) + 4;
	}
	if(s[i] == '*' && s[i + 1] == '.' && s[i + 2] == '*')
	{
		if (etoile2 == 0 && nombre_charact_int == 0)
		...
		{
			i = ft_flag_etoile_zero_point_x_maj(i, etoile, etoile2, nombre_charact_int) + 4;
		}
	}
	return (i);
}