...
int		ft_ecriture_tiret_point_x_maj(int y, int tampon, int len_int)
{
	if (y > tampon)
	{
		if (tampon > len_int)
...
int		ft_parametre_x_maj(int i, int y, const char *s, unsigned int nombre_charact_int)
{
	int tampon;
	char *dest = NULL;
	char *tmp = NULL;
	...
	ft_ecriture_tiret_point_x_maj(y, tampon, len_int);
	return (1);
}
...
int		ft_flag_tiret_x_maj(int i, const char *s,unsigned int nombre_charact_int)
{
	...
	while (s[y] != 'X')
	{
		if (s[y] == '.')
		{
			len_int = ft_flag_point_x_maj(y + 1 , s,nombre_charact_int);
			ft_parametre_x_maj(i, y, s, nombre_charact_int);
			return (len_int );
		}
		y++;
	}
	...
	len_int = ft_strlen(dest);
	y = ft_atoi(tmp);
	ft_ecriture_tiret_x_maj(i, y, dest, len_int);
	return (i);
}